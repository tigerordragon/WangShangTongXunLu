(function () {
    ...
    if (form) {
      form.addEventListener("submit", function (event) {
        ...
        request("/auth/login", {
          ...
          setNotice("loginStatus", "success", "Signed in successfully.");
          renderStudentSessionSummary();
          renderLoginInfo(loadStudentAuth());
          loadStudentLoginInfo();
        });
      });
    }
  ...
  function initAdminLoginPage() {
    ...
    if (form) {
      form.addEventListener("submit", function (event) {
        ...
        setNotice("adminLoginStatus", "success", "Admin signed in successfully.");
        renderAdminSessionSummary();
      });
    }
    ...
    if (loadAdminSession()) {
      setNotice("adminLoginStatus", "info", "An admin session is already active.");
    }
  }
  ...
  function initProfilePage() {
    ...
    if (reloadBtn) {
      reloadBtn.addEventListener("click", function () {
        setNotice("profileSummary", "info", "Reloading profile...");
        loadProfile();
      });
    }

    if (!getStudentToken()) {
      setNotice("profileSummary", "warning", "Please sign in first.");
      return;
    }
  ...
  function initSearchPage() {
    ...
    if (getStudentToken()) {
      setNotice("searchStatus", "info", "Student session detected. You can search now.");
    } else {
      setNotice("searchStatus", "warning", "Please sign in first.");
    }
  }
